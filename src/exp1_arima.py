...
def get_arima_dataset(filepath, all_country):
    ...
    for country in all_country:
        ...
        df.append(cur_df.loc[:, ['date', 'day_no', 'country', 'cases']])
    df = pd.concat(df)
...
TEST_SIZE = 30
DEFAULT_SIZE = 90  # consider first 90 days by default
df_arima = get_arima_dataset(csv_processed_with_null_path, countries)


for valid_country in countries:
    df_arima_country = df_arima[df_arima['country'] == valid_country]
    df_arima_country = df_arima_country.drop(['country', 'day_no'], axis=1)
    metric_scores = []

    for p_days in [pretrain_days[-1]]:  # TODO: Remove last -1
        TRAINING_SIZE = p_days + DEFAULT_SIZE
        train = df_arima_country.iloc[0:TRAINING_SIZE]
        test = df_arima_country.iloc[TRAINING_SIZE: TRAINING_SIZE + TEST_SIZE]
        # plot_cases(train, test)
        train_list = flatten(train.values.tolist())
        non_zero_index = [i for i, x in enumerate(train_list) if x][0]

        # excluding non zero indexes
        train = train.iloc[non_zero_index:]
        arima_model = get_arima_model(train)
        prediction, confint = arima_model.predict(n_periods=int(test.size), return_conf_int=True)
        df_cf = pd.DataFrame(confint)
        plot_prediction(prediction, df_cf, test, train, p_days)
        model_predictions = {'ARIMA': prediction.values}
        metric_scores.append(get_scores(flatten(test.values.tolist()), model_predictions, p_days))

    df_scores = pd.concat(metric_scores)
    get_metric_with_mean(df_scores, 'MAPE')
