...
class StreamEvaluator(BaseSKMObject, metaclass=ABCMeta):
    ...
    def _update_metrics(self):
        ...
        for metric in self.metrics:
            ...
            elif metric == constants.MAPE:
                for i in range(self.n_models):
                    values[0].append(self.mean_eval_measurements[i].get_mean_absolute_percentage_error())
                    values[1].append(self.current_eval_measurements[i].get_mean_absolute_percentage_error())
            ...
            with open(self.output_file, 'w+') as f:
                ...
                for metric in self.metrics:
                    ...
                    #NOTE: Added MAPE
                        ...
                        for i in range(self.n_models):
                            header += ',mean_mape_[{0}],current_mape_[{0}]'.format(self.model_names[i])
                    elif metric == constants.MAE:
                        for i in range(self.n_models):
    ...
    def evaluation_summary(self):
        ...
        for i in range(self.n_models):
            ...
            #NOTE: Added MAPE
            if constants.MAPE in self.metrics:
                print('{} - MAPE          : {:.4f}'.format(
                    self.model_names[i],
                    self._data_buffer.get_data(metric_id=constants.MAPE, data_id=constants.MEAN)[i]))
